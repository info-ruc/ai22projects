...
class DataManager(object):
    ...
    def generate_all_train_batches(self):
        ...
        while not self.current_train_offset + self.batch_size > self.test_offset:
            ...
            print(np.array(raw_batch_la))
            print(np.reshape(np.array(raw_batch_la), (-1)))
            
            is_same_size=True
            judge=np.array(raw_batch_la)
            sub_size=len(judge[0])
            for i in range(1,len(np.array(raw_batch_la))):
                if len(judge[i])!=sub_size:
                    is_same_size=False
                    break
